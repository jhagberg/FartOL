...
function doIngest(
  ...
  if (data.classes.length === 0 && data.courses.length > 0) {
    ...
    for (const cr of data.courses) {
      ...
      // Gemini review G2-001: the `id` field here is a placeholder to
      // satisfy ParsedCourseData['classes'] (which requires `id: string`).
      // The actual DB id is generated by the insert loop below at line
      // 89-101 via crypto.randomUUID() — for both synth and XML-parsed
      // classes, the loop ignores the source id and generates fresh. The
      // course→class wiring downstream uses `name` (not id), so this
