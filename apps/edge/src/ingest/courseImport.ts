...
function doIngest(
  ...
  // Phase 2.0 D-CO-1: course-only fallback. When the source XML has zero
  // <Class> elements but has <Course> elements (Condes/Purple Pen "courses
  // only" exports — typical for 4-klubbs/training events), synthesize a
  // 1:1 class-per-course so the WalkupModal Bana picker has rows to show
  // and so the existing course→class FK chain still resolves. Each
  ...
  if (data.classes.length === 0 && data.courses.length > 0) {
    ...
    for (const cr of data.courses) {
      ...
      synthesised.push({ id: `auto-${cr.name}`, name: cr.name, short_name: null });
    }
  ...
  for (const cr of effectiveCourses) {
    const id = crypto.randomUUID();
