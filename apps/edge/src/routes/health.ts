...
// scaffolding in plan 03. node_id is sourced from $FARTOL_NODE_ID (defaults
...
import type { HealthDTO } from '@fartol/shared-types';

export async function registerHealthRoute(app: FastifyInstance): Promise<void> {
  app.get('/api/health', async (): Promise<HealthDTO> => {
    const nodeId = process.env.FARTOL_NODE_ID ?? 'local-dev';
