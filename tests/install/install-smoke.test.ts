...
//      drops the `fartol` bin in PATH and `fartol` boots cleanly.
//
//   2. C-H4 layout assertion (independent of the shell script): run the
//      raw `npm install --prefix <tmpdir> -g <tarball>` invocation and
//      verify that the bin resolves at `$tmpdir/lib/node_modules/.bin/fartol`
...
// (the root `test:install` script wires this up; `bash scripts/build-fartol.sh`
...
function resolveTarball(): string {
  // The build script aliases the pnpm-named tarball to `dist/fartol-<version>.tgz`
  // so this test (and operators following the README) refer to a stable name.
  // Match only fartol-VERSION.tgz, NOT fartol-edge-VERSION.tgz (the pnpm-default
  // scoped name) which npm would attempt to resolve as a git url when passed
  // as a relative path.
  const list = execSync(
    'ls dist/fartol-[0-9]*.tgz 2>/dev/null || ls dist/fartol-*.tgz 2>/dev/null',
    { encoding: 'utf8' }
  )
    .split('\n')
    .map((line) => line.trim())
    .filter((line) => line.length > 0 && /\/fartol-\d/.test(line));
  list.sort();
  const relative = list[list.length - 1] ?? '';
  if (!relative) {
    throw new Error(
      'tests/install: no tarball under dist/. Run `bash scripts/build-fartol.sh` first.'
    );
  }
...
test('install-smoke: tarball boots cleanly via the smoke script', () => {
  ...
  const out = execSync(`bash apps/edge/scripts/install-smoke.sh '${tarball}'`, {
    encoding: 'utf8',
    env: { ...process.env, FARTOL_SMOKE_PORT: port },
...
test('C-H4 regression: install-smoke BIN path resolves to a real executable BEFORE invocation', () => {
  const tarball = resolveTarball();
  const tmpRoot = mkdtempSync(path.join(tmpdir(), 'fartol-bin-check-'));
  try {
    // --prefix + -g together is what produces the global-prefix bin layout.
    // Dropping -g shifts the bin to $tmpRoot/node_modules/.bin/fartol which
    ...
    // and points into $prefix/lib/node_modules/<scope>/<name>/dist/bin/fartol.cjs.
    const BIN = path.join(tmpRoot, 'bin', 'fartol');
    assert.ok(
      existsSync(BIN),
