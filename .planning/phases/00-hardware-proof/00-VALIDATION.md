...
_Task IDs are placeholders (`TBD-_`). Planner replaces with concrete IDs (e.g., `0-01-01`) per plan ownership; the row's `Requirement`/`Test Type`/`Automated Command` columns stay verbatim.\*
...
_No test-framework install required (node:test is built-in). lefthook + commitlint install separately (not test infra)._
