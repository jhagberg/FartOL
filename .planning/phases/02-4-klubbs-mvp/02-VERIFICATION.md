...
gaps:
  - truth: "SC#5 — Course-only model (no Klasser) works for 4-klubbs's 5-course bundle (Vit / Grön / Gul / Orange / Violett)."
    ...
    reason: |
      ...
      match between MeOS and fartOLa but does NOT instruct creation of
      the 5 classes in fartOLa (no `POST /api/competitions/:id/classes`
    ...
    artifacts:
      ...
      - path: 'docs/ops/parallel-meos-runbook.md'
        issue: "Step 7 says 'Bana picker should show the same five entries' but no step explains how to create the 5 classes in fartOLa"
    missing:
...
deferred:
  - truth: 'SC#1 — 4-klubbs 2026-05-20 runs end-to-end on fartOLa; MeOS alive but never needed.'
...
human_verification:
  - test: 'Bench-smoke green against real BSM7/8 + real MeOS + real Eventor key'
    expected: '`FARTOLA_PORT=3000 FARTOLA_DB=/var/lib/fartola/4-klubbs.db FARTOLA_SKIP_BOOT=1 bash apps/edge/scripts/bench-smoke-phase2.sh` reports 6/6 PASS against the prod bridge'
  ...
  - test: 'MIP <5s latency from fartOLa walk-up → MeOS'
    expected: 'Walk-up registered in fartOLa appears on MeOS within ~5s via MeOS-side MIP poll (manual test 8 in 02-06 deferred task)'
    why_human: 'Needs real MeOS install polling our /mip endpoint at its configured cadence'
  - test: 'MOP crash-recovery import'
    expected: "Kill fartOLa → register competitor in MeOS → restart fartOLa → MeOS-side competitor appears as `source='meos'` via next MOPComplete cycle"
    why_human: 'Needs real MeOS push timing + bridge kill/restart; manual test 10 in 02-06 deferred task'
  - test: 'Hyrbricka belt+braces: both fartOLa toast AND MeOS reminder fire'
    expected: 'Card-bound runner with hired=true → MIP <entry><card hired="true">...</card></entry> reaches MeOS → MeOS surfaces its own rental reminder AND fartOLa surfaces the HyrbrickaToast at finish-readout'
  ...
  - test: 'SC5 actual remediation (operator workaround)'
    expected: 'Even with the gap above, an operator who manually creates the 5 classes in fartOLa via REST before the event can run walk-ups end-to-end; verify this workaround'
...
Wednesday 2026-05-20 with fartOLa as primary registration + readout
...
| --- | --------------------------------------------------------------------------------------------------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | 4-klubbs 2026-05-20 runs end-to-end on fartOLa; MeOS alive but never needed.                                     | DEFERRED (Wed checkpoint) | Plan 02-06 Task 4 is the production gate; not a code-path concern. Acknowledged by verification context. Code-path readiness for SC1 is the conjunction of SCs 2-6.                                                             |
| 2   | Eventor löpardatabasen import works: typing or reading known SI bricka auto-fills name+klubb in walk-up.        | VERIFIED                  | apps/edge/src/eventor/{download,parser,cache,boot,lookup}.ts all present, bin/fartola.ts L527 calls scheduleEventorBoot; e2e `walkup-eventor.spec.ts` runs 4/4 green including "bricka pre-fill" and "Bana label" cases.         |
| 3   | Every walk-up registration in fartOLa shows up in MeOS within ~5s via MIP `<entry>`.                             | VERIFIED (code-path)      | apps/edge/src/integrations/meos/mip.ts wired at server.ts:243; mip.test.ts 12 cases incl. D-MIP-3 round-trip; XSD round-trip vs pinned mip.xsd v3.0. Real ~5s latency depends on MeOS-side poll cadence (human verification).   |
| 4   | Hyrbricka flag survives round-trip: fartOLa toast at finish-readout AND MeOS reminder both fire.                 | VERIFIED (fartOLa side)    | e2e `hyrbricka.spec.ts` 1/1 green (walkup→toast→Returnerad→no re-pop→admin). MIP serializer emits `<card hired="true">` (mip.ts L273-277). MeOS side requires real-MeOS bench gate.                                             |
...
| 6   | If fartOLa killed mid-event, MeOS-side registrations done during outage are picked up via MOP on fartOLa restart. | VERIFIED (code-path)      | mop.ts D-MOP-3 auto-merge SQL (L259-294); mop.test.ts 17 cases; smoke assertion 2 exercises real POST. Restart-resume depends on the bridge bin (PRESERVED — DB-backed). Bench-checkpoint manual test 10 is the real-MeOS gate. |
...
| `apps/edge/src/eventor/{boot,cache,download,parser,lookup}.ts`               | Eventor pipeline                               | VERIFIED | All 5 modules present; scheduleEventorBoot wired in bin/fartola.ts L527                                                                  |
...
| `apps/edge/scripts/bench-smoke-phase2.sh`                                    | 6-assertion bash gate, env-var parameterized   | VERIFIED | All 6 assertions documented (L132-294); FARTOLA_SKIP_BOOT short-circuit for Task 4 prod-bridge mode; chmod 755                           |
...
| ------------------ | -------------------------------- | --------------------------------------------- | ---------- | ----------------------------------------------------------------------------------------------------------------- |
| bin/fartola.ts      | eventor/boot.ts                  | `scheduleEventorBoot(handle, {...})` L527     | WIRED      | Decorated as `app.fartolaEventor` L531                                                                             |
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------ | --------------------------------------- | ------------------------------------------------- |
| Edge test suite passes           | `pnpm --filter @fartola/edge test --test-name-pattern="eventor\|mip\|mop\|hyrbricka\|hired\|retention"` | 376 pass, 0 fail, 1 skipped             | PASS                                              |
| Web test suite passes            | `pnpm --filter @fartola/web test --run`                                                                 | 86/86 pass                              | PASS                                              |
