...
provides:
  ...
  - GET /api/eventor/status → 200 { state: ready | stale | offline | no_key, ageDays, competitorCount, fartol_dev }
  ...
  - apps/web/src/lib/stores/eventorStatus.svelte.ts: Svelte 5 rune store with state/ageDays/competitorCount/fartol_dev; soft-fails refresh to state='offline' on network error (D-EV-3); triggerEventorRefresh fires admin POST + re-fetches status
  - apps/web/src/lib/components/TweaksPanel.svelte: Eventor row with status dot + i18next-interpolated label + FARTOL_DEV-gated 'Uppdatera' button (gated on server-side status.fartol_dev — NOT import.meta.env.DEV which would be bundler-time and always false in production builds)
  - apps/edge/src/routes/dev.ts: POST /api/__dev/eventor-seed FARTOL_DEV-gated helper for e2e specs
...
tech-stack:
  ...
  patterns:
    ...
    - 'Server-side fartol_dev flag in status endpoints so the UI gate is correct in production builds (request-time process.env eval; NOT import.meta.env.DEV which is bundler-time)'
...
key-decisions:
  ...
  - "TweaksPanel's 'Uppdatera' button is gated on status.fartol_dev (server-side) rather than import.meta.env.DEV (bundler-time). The latter would always be false in production builds even when the operator boots with FARTOL_DEV=1, making the button unreachable in real ops."
  - "Added POST /api/__dev/eventor-seed (FARTOL_DEV-gated) as the e2e seeding path. The alternative — running the real Eventor fetch — would require the API key in CI and add a 5s+ network roundtrip per test setup."

patterns-established:
  - "Pattern: discriminated-union REST hit/miss for autocomplete lookups (EventorLookupHit | EventorLookupMiss)"
  - "Pattern: server-side runtime flags surfaced via /status endpoints (fartol_dev) so UI gates are correct in production builds"
...
**Walk-up modal wires the Plan-01 Eventor cache to pre-fill name + klubb on bricka scan AND on prefix-typed autocomplete; Hyrbricka checkbox + contact fieldset writes to hired_cards in the same transaction as competitors; TweaksPanel surfaces cache health with a FARTOL_DEV-gated manual refresh button.**
...
- `registerEventorRoutes` mounts `/api/eventor/lookup` + `/api/eventor/status`. Lookup enforces mutual exclusion of `si_card` and `prefix`; status derives `state ∈ {ready, stale, offline, no_key}` from the boot-marker plus `competitorCount` plus a request-time `fartol_dev` flag so the TweaksPanel admin button works in production.
...
- `eventorStatus.svelte.ts` runes store: soft-fails refresh to `state='offline'` on network error (D-EV-3); `triggerEventorRefresh` fires the FARTOL_DEV-gated admin POST then re-fetches the status to land the truth.
- `TweaksPanel` Eventor row with a status dot, i18next-interpolated label (`tweaks.eventor.ready: 'Eventor: cache OK ({{days}} dagar gammal)'`), and the FARTOL_DEV-gated "Uppdatera" button.
...
- `apps/edge/src/routes/eventor.ts` — Fastify plugin `registerEventorRoutes` mounting GET `/api/eventor/lookup` (mutual-exclusion of si_card and prefix, 400 missing_query / conflicting_query) and GET `/api/eventor/status` (state derived from boot marker + competitor count + request-time `fartol_dev`).
- `apps/edge/src/routes/eventor.test.ts` — 9 node:test cases covering hit/miss/missing/conflicting + all four state branches + the FARTOL_DEV request-time eval.
...
- `apps/edge/src/routes/dev.ts` — added POST `/api/__dev/eventor-seed` (FARTOL_DEV-gated) wrapping `ingestEventorCache` against the Plan-01 fixture.
...
- `apps/web/src/lib/components/TweaksPanel.svelte` — Eventor row with `$derived(getEventorStatusStore())`, status dot, i18next-interpolated label, and `{#if eventorState.fartol_dev}`-gated refresh button. Mounts a one-shot refresh via `$effect` on first open.
...
- **Server-side `fartol_dev` flag**: derived from `process.env.FARTOL_DEV === '1'` at request time so the TweaksPanel "Uppdatera" button shows correctly in production builds when the operator boots with FARTOL_DEV=1. `import.meta.env.DEV` would be bundler-time and always `false` in prod.
- **`/api/__dev/eventor-seed`**: FARTOL_DEV-gated dev helper for the e2e spec. Avoids requiring an Eventor API key in CI and the ~5s network roundtrip per test setup.
...
- **Fix:** Added a small FARTOL_DEV-gated `POST /api/__dev/eventor-seed` route in `apps/edge/src/routes/dev.ts` that ingests the bundled Plan-01 fixture (`competitors-sample.xml` + `clubs-sample.xml`). The e2e calls it once per test via `request.post(...)`.
...
The Eventor cache must be populated for the live walk-up flow to pre-fill from Eventor data. Already covered by Plan 02-01 (bridge boot fetches when `EVENTOR_API_KEY` is set in `.eventor-env`). For local dev / e2e the new `POST /api/__dev/eventor-seed` (FARTOL_DEV-gated) seeds from the bundled 3-record fixture.
