...
- **After every plan wave:** `pnpm --filter @fartola/edge test && pnpm --filter @fartola/web test` — full unit + integration ~30s.
...
7. **Ops runbook drill** — Read `docs/ops/parallel-meos-runbook.md` cold and confirm a non-author operator can boot fartOLa + point MeOS at /mip + /mop in <10 min. Owned by Plan 06 Task 2.
...
- **bench-smoke-phase2.sh** is the integration gate that bridges automated tests and the Wednesday manual checkpoint. Plan 06 Tasks 3 + 4 share env-var parameterization (`FARTOLA_PORT`, `FARTOLA_DB`, `FARTOLA_HOST`, `FARTOLA_SKIP_BOOT`) so the same script tests both the local-mocked-bridge case and the production-bridge case without modification.
