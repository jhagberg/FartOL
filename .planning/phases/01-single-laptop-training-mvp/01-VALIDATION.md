...
| 01-02-T2 | 02 | 0 | REQ-EVT-001, REQ-EVT-002, REQ-EVT-004, REQ-OPS-002 | T-EVENT-TAMPER | embedded migrator idempotent; events append-only; node_id persists across close→reopen cycles (cross-process restart-safety, REQ-OPS-002) | unit | `pnpm --filter @fartol/edge test` | apps/edge/src/db/{schema,events,migrate}.test.ts | ⬜ pending |
...
| 01-06-T1 | 06 | 2 | REQ-EVT-001, REQ-EVT-002 | T-SEQ-COLLISION, T-PUNCHES-WIRING-DRIFT, T-IDLE-CHANNEL-LEAK | sqlite.transaction serializes nextLocalSeq+INSERT; punches sourced from card.raceResult.punches (mirrors NdjsonEmitter line 253); no phantom 'readout:__idle__' channel | unit | `pnpm --filter @fartol/edge test` | apps/edge/src/si/eventInserter.test.ts | ⬜ pending |
| 01-06-T2 | 06 | 2 | REQ-HW-001..004, REQ-EVT-CMP-005 | T-USB-EBUSY, T-EVENT-DROP-ON-CRASH, T-PUNCHES-WIRING-DRIFT, T-IDLE-CHANNEL-LEAK | reconnect backoff; bench-replay regression including SI10-fixture payload.punches.length > 0 gate (B-1) and zero-broadcast-when-idle gate (B-2) | integration | `pnpm --filter @fartol/edge test` | apps/edge/src/si/bridge.test.ts | ⬜ pending |
...
| 01-12-T2 | 12 | 4 | REQ-UI-002, REQ-EVT-CMP-002 | T-ORPHAN-COMPETITION, T-CLIENT-XML-PARSE | deferred-POST (LOCKED, no eager-POST alternative); client+server XML gating | e2e | `FARTOL_DEV=1 npx playwright test tests/e2e/wizard.spec.ts` | tests/e2e/wizard.spec.ts | ⬜ pending |
...
| 01-15-T2b | 15 | 4 | REQ-UI-004 | T-SHARP-SVG-INPUT, T-AUTOPRINT-LOOP | sharp PNG bitmap; auto-print 400ms delay gated by activeCompetitionId !== null + competition.auto_print=true; W-7 fake-timer test deterministically asserts printerSink.print fires within 600ms (and zero-call gates when idle / auto_print=false) | unit + integration | `pnpm --filter @fartol/edge test` | apps/edge/src/print/kids-svg-to-bitmap.test.ts + apps/edge/src/si/bridge.test.ts (auto-print suite) | ⬜ pending |
...
| 01-16-T2 | 16 | 5 | REQ-EVT-CMP-008, REQ-STD-002 | — | preview + download endpoints; UI gating; route-layer @status enum gate + empty-competition returns 200 with valid empty ResultList (W-5) | e2e | `FARTOL_DEV=1 npx playwright test tests/e2e/export.spec.ts` | tests/e2e/export.spec.ts | ⬜ pending |
