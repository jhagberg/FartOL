...
function App() {
  ...
  // Routing state
  const [route, setRoute] = useState('readout'); // home | readout | results
  ...
  const simulateRead = () => {
    ...
    setFlashKey(k => k + 1);
    if (next.unknown) {
      setTimeout(() => setWalkupOpen(true), 600);
  ...
  return (
    <div className={'app ' + rootClass}>
      <aside className="sidebar">
        ...
        <button className="nav-item" disabled style={{opacity: 0.55, cursor: 'default'}}>
      ...
      {walkupOpen && currentRead?.unknown && (
        <WalkupModal
          t={t}
          cardNumber={currentRead.cardNumber}
