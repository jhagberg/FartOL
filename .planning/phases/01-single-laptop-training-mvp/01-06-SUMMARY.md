...
provides:
  ...
  - 'apps/edge/src/bin/fartol.ts — full SI bridge lifecycle (SerialTransport + reconnect-with-backoff) + --no-bridge / --serial-path / --competition-id flags'
...
tech-stack:
  added:
    - 'No new package deps — @fartol/sportident workspace dep was already added in plan 02.'
  patterns:
    - 'PATTERNS S-1: every new .ts file carries a file-header preamble citing 01-06-PLAN.md + the relevant codex finding (C-H2) + the source-of-truth line range in @fartol/sportident.'
    ...
    - 'T-PAYLOAD-DRIFT type-binding: buildCardReadPayload returns Extract<EventPayload, { event_type: "card_read" }>; the schema arm imports HalfDayClock + NdjsonPunch from @fartol/sportident, so any Phase 0 shape change surfaces as a TS compile error in Phase 1.'
...
key-files:
  created:
    - 'apps/edge/src/si/cardType.ts — 1-line re-export of @fartol/sportident inferCardType as cardTypeFromNumber.'
  ...
  modified:
    ...
    - 'apps/edge/src/bin/fartol.ts — full BridgeLifecycle class with reconnect chain + --no-bridge / --serial-path / --competition-id flags; --help text refreshed.'
...
key-decisions:
  ...
  - 'BridgeLifecycle is a class in apps/edge/src/bin/fartol.ts (not a separate module) — it has no callers outside the bin. Erasable-syntax constraint forced explicit field declarations + assignments instead of TS parameter properties (mirrors the WsClient fix from plan 03).'
...
- **Files modified:** 5 (server.ts, bin/fartol.ts, routes/dev.ts, sportident/ndjson.ts, sportident/index.ts)
...
- **Bin lifecycle: `--no-bridge` boots clean.** Smoke-verified live: `cd apps/edge && node --import tsx src/bin/fartol.ts --no-bridge --port 3099 --db-path /tmp/...` returned `GET /api/health → {"status":"ok","node_id":"...","uptime_ms":3007}` after ~3s. Plan-06 verification step 7 satisfied.
...
- `apps/edge/src/bin/fartol.ts` — BridgeLifecycle class owns SerialTransport + SiMainStation + AttachedBridge + the 250ms/500ms/1s/2s/5s reconnect chain. New flags: `--no-bridge`, `--serial-path`, `--competition-id`. --help text refreshed.
...
2. **`buildCardReadPayload` returns the SCHEMA's card_read arm, not `Omit<CardReadEvent, ...>`.** Phase 0's `CardReadEvent` interface uses `event` (not `event_type`) and `ts_ms` (not `ts`), so a literal Omit wouldn't typecheck. The schema arm imports the same component types (`HalfDayClock` + `NdjsonPunch`) from `@fartol/sportident`, so T-PAYLOAD-DRIFT detection is still TS-compile-time.
...
5. **BridgeLifecycle is a class** in `apps/edge/src/bin/fartol.ts` (not a module) because it has no callers outside the bin. Erasable-syntax constraint required explicit field declarations + body-assigns (no TS parameter properties).
...
- **Issue:** Plan 06 instructed `export { cardTypeFromNumber } from '@fartol/sportident'`. The Phase 0 function is named `inferCardType` (the file is `cardTypeFromNumber.ts` but the export inside is `inferCardType`).
- **Fix:** Re-export with rename: `export { inferCardType as cardTypeFromNumber } from '@fartol/sportident'`. Plan spec is preserved at the apps/edge surface; Phase 0 keeps its historical naming.
...
- **Found during:** Task 2 typecheck of bin/fartol.ts
...
- **Issue:** Lefthook flagged 2 files in Task 1 (bridge.ts + eventInserter.test.ts) and 2 in Task 2 (bin/fartol.ts + bridge.test.ts). ESLint flagged an unused `competitions` import in bridge.test.ts.
...
- `apps/edge/src/bin/fartol.ts`: FOUND (modified — BridgeLifecycle + new flags)
...
- `pnpm --filter @fartol/edge test`: 132 / 132 pass (9 new bridge + 5 new cardReadPayload + 5 new eventInserter tests).
- `pnpm --filter @fartol/sportident test`: 108 / 108 pass (post-toHalfDayClock-export — additive change, no Phase 0 regression).
...
- `cd apps/edge && node --import tsx src/bin/fartol.ts --no-bridge --port 3099 --db-path /tmp/x.db` + `curl http://127.0.0.1:3099/api/health` → `{"status":"ok",...}` (verification step 7).
