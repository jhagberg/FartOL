...
This runbook is the operator's playbook for running FartOL + MeOS in
...
> **Quick recovery pointer.** If FartOL crashes during the event, jump
> straight to [When something breaks → FartOL crashes mid-event](#fartol-crashes-mid-event).
...
- On the MeOS laptop, open a browser → `http://<fartol-ip>:3000/competition/<id>/readout`.
- The same readout page should load. If it does not, double-check
  `--bind-host 0.0.0.0` and the local firewall on the FartOL laptop
  (`sudo ufw status` if applicable).

### 6. Configure MeOS to talk to FartOL

On the MeOS laptop, open MeOS:

1. **Tools → Online → Configure MIP / MOP** (exact menu path varies
   slightly by MeOS version).
2. Set the **MIP URL**: `http://<fartol-ip>:3000/mip`
   - No password — D-MIP-1 (closed club LAN).
3. Set the **MOP URL**: `http://<fartol-ip>:3000/mop`
   - No password — D-MOP-4.
...
- In MeOS, list the **five 4-klubbs classes**: Vit / Grön / Gul / Orange /
  Violett.
- In FartOL, open the Walkup modal and inspect the **Bana** picker
...
- Open the FartOL TweaksPanel (Sidebar → Tweaks).
...
The deterministic gate. From the FartOL laptop, in the repo working
directory:
...
`FARTOL_SKIP_BOOT=1` tells the script to reuse the already-running prod
...
The roles are split across the two laptops. The FartOL operator is the
primary; the MeOS operator is mostly hands-off.

### Operator (you) at the FartOL laptop
...
FartOL, the `<entry>` carries `hired="true"` so MeOS's own rental-
reminder kicks in as well (belt + braces).

### Card readouts

- Cards punch into the BSM-mini → FartOL processes natively (Phase 1
  bridge path).
...
`http://<fartol-ip>:3000/competition/<id>/hyrbrickor` — the admin
...
### FartOL crashes mid-event

Symptoms: the FartOL UI tab goes blank or the readout stops updating;
`systemctl status fartol` shows `failed` or `inactive`.

1. Restart the bridge with the **same competition id**:
   ```
   sudo systemctl restart fartol
   ```
   (or the equivalent `fartol --port 3000 ...` invocation if you're not
   on systemd).
...
4. **D-LIM-1 manual workaround.** MOP `<cmp>` does **NOT** carry the
   ...
   in FartOL's Walkup modal (Hyrbricka checkbox + at least phone OR
...
1. FartOL keeps running standalone. Phase 1 functionality (registration,
...
4. FartOL's MIP backlog catches MeOS back up automatically: MeOS's
...
3. If you fix internet / key mid-event, press **Uppdatera** in TweaksPanel
   (FARTOL_DEV mode) to force a refresh and re-enable autocomplete.

### LAN connectivity drops between laptops

Symptoms: MIP polls stop reaching FartOL; MeOS desk shows runners
appear minutes late.
...
2. When connectivity returns:
   - MeOS resumes polling `/mip` and catches up on missed `<entry>` via
     `lastid > 0`.
   - FartOL resumes accepting `/mop` POSTs. MOP auto-merge runs on the
     next `<MOPComplete>`.
3. No data loss — both systems' state is durable independently.

### Reader fails / wrong serial port

Symptoms: card beep doesn't open the Walkup modal; `journalctl -u fartol`
...
2. Common causes (and fixes):
   - **/mip returns invalid XML** → bridge isn't fully up; wait 10s
     and retry.
   - **MOP POST returns ERROR** → check `journalctl -u fartol` for an
...
3. **If you can't fix the smoke before the event start**, the safe
   ...
   and reconcile in FartOL post-event by re-typing from MeOS's exported
   results.
...
2. **Cross-check FartOL vs. MeOS.** Both systems should agree on
   competitors + finish times. If they diverge, **FartOL is
...
recovery. **Workaround:** re-enter those rentals in FartOL's Walkup
...
FartOL. Mismatch = the MIP entry is rejected silently. See Step 7 in
...
### URLs (substitute `<fartol-ip>` and `<id>` from your boot)

| What                              | URL                                                     |
| --------------------------------- | ------------------------------------------------------- |
| Readout view                      | `http://<fartol-ip>:3000/competition/<id>/readout`      |
| Registration desk (kids line)     | `http://<fartol-ip>:3000/competition/<id>/registration` |
| Hyrbrickor admin backstop         | `http://<fartol-ip>:3000/competition/<id>/hyrbrickor`   |
| TweaksPanel (Eventor refresh etc) | Sidebar → Tweaks from any competition route             |
| Bridge health probe               | `http://<fartol-ip>:3000/api/health`                    |
| MIP poll URL (MeOS configures)    | `http://<fartol-ip>:3000/mip`                           |
| MOP push URL (MeOS configures)    | `http://<fartol-ip>:3000/mop`                           |
| IOF XML export                    | `http://<fartol-ip>:3000/api/competitions/<id>/export`  |
...
# Force Eventor refresh (FARTOL_DEV=1 only)
...
Fields to fill: MIP URL + MOP URL (both `http://<fartol-ip>:3000/...`)
